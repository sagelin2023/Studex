...
export default function ItemCard({title, price, category, condition, image_url, created_at})
{
  return (
        ...
        >
            <img src={image_url ? image_url: ryan} alt= {title} className="w-full h-48 object-cover"/>
                <div className="p-5">
                    ...
                    <p className="text-xs text-gray-400 mt-2 flex">
                        <Clock className ="mr-1" size ={13}></Clock>Posted {new Date(created_at).toLocaleDateString()}
                    </p>
                </div>
        </motion.div>
    )
