...
export default function App() {
  return (
    <main className="min-h-screen bg-gradient-to-b from-white via-gray-50 to-gray-100 text-gray-800 font-inter">
      {/* Navbar */}
      <nav className="flex justify-between items-center px-8 py-4 border-b border-gray-200 bg-white/70 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="flex items-center gap-6 text-sm font-medium">
          ...
          <button className="px-4 py-2 bg-indigo-600 text-white rounded-full font-semibold hover:bg-indigo-700 transition">
            Get Started
          </button>
        </div>
      </nav>
      ...
      <section className="py-20 text-center bg-indigo-600 text-white">
        ...
        <button className="px-8 py-3 bg-white text-indigo-600 font-semibold rounded-full hover:bg-indigo-50 transition">
          Get Started Now
        </button>
      </section>
      ...
      <footer className="py-8 text-center text-gray-500 text-sm">
        © {new Date().getFullYear()} Studex — Built with ❤️ by VT Students.
      </footer>
    </main>
  );
}
